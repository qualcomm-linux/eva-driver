...
#include "cvp_glymur_hal.h"

extern struct cvp_hal_ops hal_ops;

void __check_tensilica_in_reset_glymur(struct iris_hfi_device *device)
{
	u32 xtss_reset_ro = 1;
...
void setup_dsp_uc_memmap_vpu5_glymur(struct iris_hfi_device *device)
{
...
void interrupt_init_iris2_glymur(struct iris_hfi_device *device)
{
	u32 mask_val = 0;
...
int __check_ctl_power_on_glymur(struct iris_hfi_device *device)
{
	u32 reg;
...
int __check_core_power_on_glymur(struct iris_hfi_device *device)
{
	u32 reg;
...
int __power_on_controller_glymur(struct iris_hfi_device *device)
{
	int rc = 0;
...
int __power_on_core_glymur(struct iris_hfi_device *device)
{
	int rc = 0;
...
int __power_off_core_glymur(struct iris_hfi_device *device)
{
	...
	if (!(value & 0x80000000)) {
		...
		__disable_gdsc(device, "core");
		msm_cvp_disable_unprepare_clk(device, "core0");
		msm_cvp_disable_unprepare_clk(device, "core_freerun");
		return 0;
	}
...
int __power_off_controller_glymur(struct iris_hfi_device *device)
{
...
void __print_sidebandmanager_regs_glymur(struct iris_hfi_device *device)
{
...
int __enable_hw_power_collapse_glymur(struct iris_hfi_device *device)
{
	int rc = 0, loop = 10;
	u32 reg_gdsc;
...
int __set_registers_glymur(struct iris_hfi_device *device)
{
...
void __print_reg_details_errlog3_low_glymur(u32 val)
{
	u32 mid, sid;
	...
	dprintk(CVP_ERR, "Sub-client:%s, SID: %d\n", mid_names_glymur[mid],
		sid);
}

void __dump_noc_regs_glymur(struct iris_hfi_device *device)
{
#ifndef USE_PRESIL42
	u32 val = 0, config;
	...
	__print_reg_details_errlog3_low_glymur(val);
...
void __noc_error_info_iris2_glymur(struct iris_hfi_device *device)
{
	struct msm_cvp_core *core;
...
int set_glymur_hal_functions(void)
{
	hal_ops.interrupt_init = interrupt_init_iris2_glymur;
	hal_ops.setup_dsp_uc_memmap = setup_dsp_uc_memmap_vpu5_glymur;
	hal_ops.power_off_controller = __power_off_controller_glymur;
	hal_ops.power_off_core = __power_off_core_glymur;
	hal_ops.power_on_controller = __power_on_controller_glymur;
	hal_ops.power_on_core = __power_on_core_glymur;
	hal_ops.noc_error_info = __noc_error_info_iris2_glymur;
	hal_ops.check_ctl_power_on = __check_ctl_power_on_glymur;
	hal_ops.check_core_power_on = __check_core_power_on_glymur;
	hal_ops.print_sbm_regs = __print_sidebandmanager_regs_glymur;
	hal_ops.enable_hw_power_collapse = __enable_hw_power_collapse_glymur;
	hal_ops.set_registers = __set_registers_glymur;
	hal_ops.dump_noc_regs = __dump_noc_regs_glymur;
	hal_ops.check_tensilica_in_reset = __check_tensilica_in_reset_glymur;
	return 0;
}
